...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, currentUser, roles, onLogout, pendingItemCount = 0, pendingVendorCount = 0, pendingBudgetCount = 0 }) => {
  ...
  const menuItems: { id: NavigationTab; label: string; icon: React.ReactNode; module?: ModuleType; adminOnly?: boolean }[] = [
    ...
    { 
      id: 'workflow_v2', 
      label: 'Workflow (V2)', 
      module: ModuleType.WORKFLOW_V2,
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg> 
    },
    { 
      id: 'item_approval', 
      label: 'Item Approval', 
      module: ModuleType.ITEM_APPROVAL,
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 
    },
    { 
      id: 'vendor_approval', 
      label: 'Vendor Approval', 
      module: ModuleType.VENDOR_APPROVAL,
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 
    },
    { 
      id: 'budget_approval', 
      label: 'Budget Approval', 
      module: ModuleType.BUDGET_APPROVAL,
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 
    },
    { 
      id: 'masters', 
        ...
        <nav className="space-y-1">
          {filteredMenuItems.map((item) => (
            ...
            >
              ...
              {item.id === 'item_approval' && pendingItemCount > 0 && (
                <span className="w-2.5 h-2.5 rounded-full bg-amber-400 flex-shrink-0" title={`${pendingItemCount} pending`} />
              )}
              {item.id === 'vendor_approval' && pendingVendorCount > 0 && (
                <span className="w-2.5 h-2.5 rounded-full bg-amber-400 flex-shrink-0" title={`${pendingVendorCount} pending`} />
              )}
              {item.id === 'budget_approval' && pendingBudgetCount > 0 && (
                <span className="w-2.5 h-2.5 rounded-full bg-amber-400 flex-shrink-0" title={`${pendingBudgetCount} pending`} />
              )}
            </button>
          ))}
        </nav>
