...
import { ItemApprovalViewModal } from './ApprovalDetailModals';
...
const ItemApproval: React.FC<ItemApprovalProps> = ({ masters, users, currentUser, onAction, refetchMasters }) => {
  ...
  const runBulkReview = async () => {
    ...
    for (const p of rows) {
      try {
        await apiPatch(`masters/Item/${p.masterId}/workflow`, { action: 'completeReview' });
      } catch {
  ...
  const runBulkApprove = async () => {
    ...
    for (const p of rows) {
      try {
        await apiPatch(`masters/Item/${p.masterId}/workflow`, { action: 'approve' });
      } catch {
        failed.push(p.masterName || p.masterId);
      }
    }
  ...
  const handleAction = async (
    ...
    try {
      await apiPatch(`masters/Item/${masterId}/workflow`, {
        action,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
      ...
      <div className="p-6">
              ...
              >
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setViewItemId(p.masterId)}
        ...
        <ApprovalActivityLog items={activityLog} users={users} onView={(id) => setViewItemId(id)} />
      </div>
      {viewItemId && (
        <ItemApprovalViewModal masters={masters} itemId={viewItemId} onClose={() => setViewItemId(null)} />
      )}
    </div>
  );
};
